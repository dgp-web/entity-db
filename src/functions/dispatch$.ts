...
export async function dispatch$<TEntityTypeMap extends EntityTypeMap>(
    ...
    /**
     * The total result of this operation
     */
    const entityTypeBulkUpdateMap: KVS<PouchDB.Core.PutDocument<any>> = {};

    /**
     * Store all docs or bulk-get results when querying the DB
     */
    let queryResult: PouchDB.Core.AllDocsResponse<any> | PouchDB.Core.BulkGetResponse<any>;
    /**
     * Store entities for processing in the intermediate steps
     */
    let entityCache: Array<any>;

    // Clear
    if (action.clear) {
        const entityTypesToClear = action.clear as ClearEntityActionParamsList<TEntityTypeMap, any>;
        let queryResult = await dbConnection.allDocs<any>({keys: entityTypesToClear as Array<string>});
    ...
    if (action.remove) {
        ...
        queryResult = await dbConnection.bulkGet({docs: entityTypesFromWhichToRemoveKeys.map(x => ({id: x}))});
        entityCache = _.flatten(queryResult.results.map(x => x.docs.map(doc => {
        ...
        entityTypesFromWhichToRemoveKeys.map(async entityType => {
            ...
            const currentState = entityCache.find(x => x._id === entityType) as any;
    ...
    if (action.set) {
        ...
        queryResult = await dbConnection.bulkGet({
            docs: entityTypesFromWhichToSetKeys.map(x => {
                return {id: x} as any;
            })
        });
        entityCache = _.flatten(queryResult.results
            .map(x => {
